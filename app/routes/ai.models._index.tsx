...
export function meta({}: Route.MetaArgs) {
  const title = "Compare AI Image Models & Features | tuziyo"
  const description =
    "Compare Nano Banana, Nano Banana Pro, Nano Banana 2, Seedream 5 Pro, and GPT Image 2 by resolution, references, credits, and best use case."
      ...
      <section className="model-index-hero">
        <div className="model-page-shell">
          <nav className="model-breadcrumbs" aria-label="Breadcrumb">
            <Link to="/">Home</Link>
            <span>/</span>
            <span>AI Image Models</span>
          </nav>
          <div className="model-index-hero-copy">
            <h1>Find the right AI image model for the work.</h1>
            <p>
              Compare five creative models by resolution, reference capacity, starting credits, and
              the kind of brief each one handles best.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="model-index-compare">
        <div className="model-page-shell">
          <div className="model-section-heading model-section-heading-left">
            ...
            <p>
              This table reflects the current product controls rather than general claims about
              provider APIs.
            </p>
          </div>
          <div
            ...
            tabIndex={0}
            aria-label="Scrollable AI image model comparison"
          >
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                {models.map(model => (
                  <tr key={model.slug}>
                    <th>
                      <Link to={`/ai/models/${model.slug}`}>{model.name}</Link>
                      <span>{model.provider}</span>
                    </th>
                    <td>
                      {model.stats.find(stat => stat.label === "maximum resolution")?.value ??
                        "1K class"}
                    </td>
                    <td>
                      {model.stats.find(stat => stat.label === "reference images")?.value ?? "—"}
                    </td>
      ...
      <section className="model-index-guide">
        <div className="model-page-shell">
          ...
          <div className="model-index-guide-grid">
            <article>
              <span>01</span>
              <h3>Need fast everyday edits?</h3>
              <p>Start with Nano Banana for low-friction drafts and reference-guided changes.</p>
              <Link to="/ai/models/nano-banana">Explore Nano Banana</Link>
            </article>
            <article>
              <span>02</span>
              <h3>Need a flexible all-rounder?</h3>
              <p>
                Nano Banana 2 balances formats, resolution, references, search, and reasoning
                controls.
              </p>
              <Link to="/ai/models/nano-banana-2">Explore Nano Banana 2</Link>
            </article>
            <article>
              <span>03</span>
              <h3>Need a complex final asset?</h3>
              <p>
                Compare Nano Banana Pro's production focus with GPT Image 2's long briefs and batch
                exploration.
              </p>
              <Link to="/ai/models/nano-banana-pro">Compare professional options</Link>
            </article>
          </div>
        </div>
      </section>
      ...
      <section className="model-final-cta">
        <div className="model-page-shell">
          <div className="model-final-cta-card">
            ...
            <p>
              Start with a draft, compare the result, and spend higher settings only where they add
              value.
            </p>
